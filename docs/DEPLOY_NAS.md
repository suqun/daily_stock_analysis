...
QQ 机器人使用 OpenClaw 部署，与 Web 服务集成。
...
QQ_OPENCLAW_SECRET=你的SECRET
QQ_OPENCLAW_TOKEN=你的TOKEN

# 管理员 QQ 号
QQ_ADMIN_QQ=你的QQ号

# 可选配置
QQ_ENABLE_PRIVATE_CHAT=true
QQ_MSG_RATE_LIMIT=1.0
```
...
```bash
cd /volume1/docker/stock
docker compose -f daily_stock_analysis/docker/docker-compose.yml up -d openclaw
```
...
```bash
# 重启 OpenClaw
docker restart stock-openclaw

# 查看日志
docker logs stock-openclaw

# 停止 OpenClaw
docker stop stock-openclaw
```

### 5. Webhook 说明

- QQ 机器人 Webhook 路径：`/qq/webhook`
- 当 OpenClaw 收到消息时，会转发到 stock-server
...
# 启动指定服务（不含 OpenClaw）
docker compose -f daily_stock_analysis/docker/docker-compose.yml up -d server zsxq
